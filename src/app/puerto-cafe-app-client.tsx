...
export default function PuertoCafeAppClient({
  ...
  const filteredCafes = useMemo(() => {
    return cafes.filter((cafe) => {
      if (zoneFilter && zoneFilter !== 'Todos' && cafe.zone !== zoneFilter) return false;
      
      if (modeFilter && modeFilter !== 'Todos') {
          const filterSlug = modeFilter.toLowerCase().replace(/\s+/g, '-');
          if (!cafe.categoryIds.includes(filterSlug)) return false;
      }

      if (priceFilter && priceFilter !== 'Todos' && cafe.price !== priceFilter) return false;

      return true;
    });
  }, [cafes, zoneFilter, modeFilter, priceFilter]);

  // 2. Handler para resetear filtros (Editorial Reset)
